...
module.exports = function(passport) {
	...
	// allows us to pass back the entire request to the callback
		...
		User.findOne({
			...
			if (user) {
				...
				newUser.local.password = newUser.generateHash(password); // use
																			// the
																			// generateHash
																			// function
																			// in
		...
		User.findOne({
			...
			if (user) {
				...
				newUser.local.password = newUser.generateHash(password); // use
																			...
																			// user
																			// model
				newUser.student.displayname = req.body.display_name;
		...
		User.findOne({
			...
			if (!user)
				return done(null, false, req.flash('loginMessage',
						'No user found.')); // req.flash is the way to set
	...
	//Authenticate Via OpenID (Google)
	passport.use(new StrategyGoogle({
	    clientID: '855763026739-sf78ijiskv3su5usde6798niogacahn3.apps.googleusercontent.com', //GOOGLE_CLIENT_ID,
	    clientSecret: '57cJTvT4J70VQDPeVfHWlZ6p',//GOOGLE_CLIENT_SECRET,
